...
4. Where do app-level mocks live? `test/mocks/@app/...` isn't an alias and doesn't mirror the import path. Its consumers mock a relative specifier. This also decides `@/app/[planner]/_components` (`useCanWrite`, `usePlannerContext`), which 7 files mock ad-hoc.
