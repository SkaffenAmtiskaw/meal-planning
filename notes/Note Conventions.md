...
A note's `type` and `status` say what should happen to it next. This is the starting spec for agents that each take one kind of note and do its next step. Only `architect`, the `/shape` skill and the `/implement` skill exist today; the other agent names are placeholders.
...
| any `spec` / `ready` | Check the note against the code, add ⚠️ Check Drift callouts, update `confirmed` | unchanged, or add `blocked-by` | `/check-drift` skill *(planned)* |
