...
2. Route each unit of work — For every module, and for any sub-task or ad hoc request that arises during the step, run the DELEGATION DECISION above before delegating anything.
3. Delegate to `@develop` — Each handoff must include the following 8 elements:
...
You MUST NOT call `@develop` without these 8 elements. You may only delegate ONE file pair at a time. Never combine multiple files or issues in a single handoff. Parallel subagents are fine if fully independent; never a single handoff spanning multiple files.
4. Delegate to `@resolve` — for anything needing reasoning but no new test coverage. Each handoff must include:
   - Target file(s)
...
5. Delegate to `@apply` — for fully pre-resolved, zero-interpretation changes. Each handoff must include:
   - Target file(s)
...
6. Review results — When any subagent returns, verify:
...
7. Gate — Once all work in the step is complete and verified, run @cleanup. Then stop. Present exactly: "Please verify: [acceptance criteria]. Reply 'confirmed' when ready to continue."